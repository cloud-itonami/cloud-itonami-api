import {prepareLauncher,prepareOperatorLaunch,operatorStatus,executeOperator} from './launcher.js';
...
export default {async fetch(request,env){try{
 ...
 if(request.method==='GET'){const project=url.searchParams.get('project');const data=await snapshot(env,project,url.searchParams.get('vault'),principal,url.searchParams.get('safe'));data.operatorGrant=await operatorStatus(env,project,principal);return json(data);}
 ...
 if(input.action==='operator-execute')return json(await executeOperator(env,input,principal));
 if(['operator-launch','operator-start'].includes(input.action))return json(await prepareOperatorLaunch(env,input,principal));
